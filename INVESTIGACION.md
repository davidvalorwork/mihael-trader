...
| Embeddings | **BGE-M3** o **Qwen3-Embedding-0.6B**, corriendo localmente (CPU) | Gratis e ilimitado, sin llamadas a API. BGE-M3 es lo que usan hoy la mayoría de stacks RAG en producción según la verificación. Sin equivalente gratuito afinado a finanzas todavía — se pierde la ventaja de voyage-finance-2. |
...
7. Construir el pipeline RAG (embeddings BGE-M3 locales, chunking, hybrid search + reranking sobre Qdrant) — hoy la infraestructura (Qdrant, SearXNG) está corriendo pero sin código que la use.
