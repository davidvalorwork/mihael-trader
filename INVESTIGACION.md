...
| Capa | Recomendación (con versión confirmada 5-ago-2026) | Alternativa |
|---|---|---|
| Despliegue | **Proceso Node.js/TypeScript local en la PC personal de tu familiar/amigo** (Windows), supervisado por **pm2 v7.0.3** (reinicio ante fallos) + **WinSW** (servicio de Windows con arranque automático) | Migrar a Vercel/nube más adelante si se decide dar acceso 24/7 sin depender de que su PC esté encendida |
| Orquestación de agentes | LangGraph (como TradingAgents) o Claude Agent SDK / MCP nativo — corriendo dentro del mismo proceso local | AutoGen, CrewAI |
| LLM (decisión crítica) | Claude vía **API de pago** (llamada saliente desde la PC local hacia Anthropic o Vercel AI Gateway) | GPT-5.x, Gemini 3.x vía AI Gateway |
| LLM (volumen alto / bajo riesgo) | Modelo abierto vía proveedor de inferencia (Together/Fireworks/DeepInfra) — p. ej. Qwen3.7, DeepSeek V4 Flash | Modelo pequeño local (Qwen3.7 7B/14B, gpt-oss-20b) si su PC tiene GPU y se quiere evitar costo de API — optimización posterior |
...
- **La política de Anthropic sobre uso de suscripción para agentes de terceros ha cambiado 5 veces en 2026** — no construir la ejecución de producción del bot sobre esa base; usar API de pago para lo crítico.
...
2. Confirmar con él si su PC va a quedarse encendida de forma continua (para uptime alto) o si el bot es más bien algo que se revisa cuando la prende — esto decide si vale la pena configurar arranque automático (sección 6.5) desde el día uno.
3. Coordinar el momento de la autorización OAuth inicial en su propia PC (necesita su teléfono con la app de Robinhood y su login — sección 6.2/6.5) y decidir cómo se cifra el token en su disco (DPAPI de Windows / `keytar`).
4. Hacer una prueba en vivo mínima del MCP de Robinhood (`review_equity_order` / `place_equity_order`) específicamente para confirmar si acepta `order_type=stop_loss` — esto determina si se necesita el stop-loss sintético desde el inicio.
5. Confirmar alcance inicial: ¿solo acciones (lo que ya cubre el MCP de Robinhood) o también cripto (requeriría integrar además la Crypto Trading API por separado)?
6. Decidir el enrutamiento de LLM: qué agentes usan API de pago (Claude) y cuáles usan un modelo abierto barato vía Together/Fireworks/DeepInfra, y configurar el Vercel AI Gateway para ambos (esto sigue siendo válido corriendo desde un proceso local — son llamadas salientes a APIs en la nube).
7. Definir qué fuentes de datos de mercado/noticias se van a pagar/contratar (Alpha Vantage, Polygon, NewsAPI, etc.) — ninguna fue evaluada en detalle aquí porque no se especificó presupuesto.
8. Definir el mecanismo de monitoreo remoto y actualización de código (sección 6.5) antes de dejarlo operando con dinero real sin que estés presente.
9. Prototipar el flujo Telegram (polling) → confirmación → Robinhood MCP de punta a punta con una sola señal simulada, corriendo ya en su PC, antes de construir el pipeline RAG y de análisis técnico completos.
