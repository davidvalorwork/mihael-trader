...
- **Sin botones**: a diferencia del patrón `InlineKeyboardMarkup` de la sección 4.1/4.3, este plugin no expone una herramienta de teclado inline. La confirmación de una orden real se vuelve **conversacional** — el asistente manda la propuesta con `reply` y espera una respuesta en lenguaje natural ("sí"/"no", "adelante", "cancela") antes de proceder — no un `callback_query` de botón.
...
- **El plugin de Telegram no expone botones** — la confirmación de una orden real depende de que el asistente interprete correctamente una respuesta en lenguaje natural ("sí", "cancela", etc.). Ser conservador: si la respuesta es ambigua, volver a preguntar en vez de asumir una confirmación.
