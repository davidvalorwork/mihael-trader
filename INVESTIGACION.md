...
> Documento de investigación previo a la implementación. Última actualización: 5 de agosto de 2026 (v9 — se añadió análisis técnico, elección LLM por suscripción vs API, detalle de copytrading/stop-loss/cierre de posiciones en Robinhood, el escenario específico de operar la cuenta de un familiar/amigo sin cobro, una **pasada de verificación doble** de todas las librerías/repos de GitHub recomendadas, la sección 1.5 sobre **búsqueda web de IA en vivo**, la sección 8 rehecha como **stack 100% gratuito**, la decisión de aislar **todo el sistema dentro de WSL2** — incluyendo Claude Code mismo —, y la decisión final de usar el **plugin oficial de Telegram de Claude Code** en vez de un bot propio, sin Ollama ni enrutador de LLM separado).
...
10. **Confirmaste que todo debe ser gratuito — esto reemplazó casi todas las recomendaciones de pago de este documento.** Rehice la sección 8 (stack tecnológico) bajo ese criterio: embeddings y reranking corriendo localmente y gratis (BGE-M3, bge-reranker), vector DB autohospedada (Qdrant vía Docker), datos de mercado/noticias vía Finnhub + Alpaca (niveles gratuitos) + GDELT, sentimiento social vía **Agent Reach** (lee Twitter/Reddit sin necesitar sus APIs oficiales, que hoy están cerradas o con aprobación de semanas), y búsqueda web vía SearXNG autohospedado. **Nota**: el punto de "LLM vía rotación de niveles gratuitos (Groq/Gemini/OpenRouter) con Ollama de respaldo" que originalmente iba aquí **quedó reemplazado por el punto 11** — ya no aplica.
11. **Decisión final sobre el LLM: Claude Code directamente, sin Ollama ni rotación de APIs gratuitas.** Dijiste que vas a usar Claude Code como el agente, así que no hace falta el enrutador entre Groq/Gemini/OpenRouter ni el respaldo local con Ollama que se investigó en las secciones 3.1–3.5 y 8/8.1 para el escenario de un bot autónomo separado — esas secciones se mantienen como historial de la investigación, pero la decisión vigente está en la sección 3.6. Esto también resuelve limpio el requisito de "todo gratis" para esta capa: no hay un costo de API nuevo que gestionar, es tu uso normal de Claude Code.
12. **Decisión final sobre Telegram: el plugin oficial de Claude Code (`telegram@claude-plugins-official`), no un bot propio con grammY.** Ya está disponible en tu instalación de Claude Code — conecta los mensajes de Telegram directamente al contexto de la sesión (herramientas `reply`/`react`/`edit_message`, sin botones — la confirmación de una orden es conversacional). Ver sección 4.5 para el detalle y los pasos de configuración (son slash commands dentro de una sesión de `claude`, no algo que un script pueda automatizar).
13. **Todo el sistema —incluyendo Claude Code mismo— vive dentro de WSL2, aislado de la instalación real de Windows.** `install.ps1` pasó de instalador completo a un bootstrap que solo asegura WSL2 y clona el repo ahí dentro; el trabajo real (Node.js, Docker Engine nativo, Agent Reach, `bun`, el CLI de Claude Code) lo hace `scripts/wsl-setup.sh`, corriendo en Linux. Ver sección 6.6.
...
### 3.6 Decisión final: Claude Code directamente, sin enrutador de LLM

Todo lo de 3.1–3.5 responde a la pregunta "¿qué LLM contrata/opera este sistema por su cuenta?" — la respuesta correcta si se estuviera construyendo un bot autónomo separado. Pero como quedó establecido en CLAUDE.md, **Claude Code es el agente**: no hay un proceso Node.js aparte haciendo llamadas a Groq/Gemini/OpenRouter ni a la API de pago de Anthropic — el razonamiento y las decisiones las toma directamente esta sesión de Claude Code, con las herramientas del proyecto (indicadores, risk-check, MCP de Robinhood, Agent Reach, plugin de Telegram) disponibles como tools.

Esto simplifica el requisito de "todo gratis" de la sección 8 para esta capa específica: no hay un costo de API *nuevo* que gestionar para este proyecto — el costo es el de tu uso habitual de Claude Code (suscripción o API, lo que ya tengas configurado), no un cargo adicional por token específico del trading. La investigación de 3.1–3.5 sobre inestabilidad de política y expiración de tokens en operación desatendida sigue siendo relevante *si algún día* este sistema necesitara correr como agente autónomo 24/7 sin supervisión — hoy no es ese el diseño.
...
### 4.5 Decisión final: plugin oficial de Claude Code, no un bot propio con grammY

Todo lo anterior (4.1–4.4) describe cómo se vería construir el bot desde cero — es la base correcta si algún día hiciera falta un bot standalone. Pero para este proyecto, donde Claude Code ya es el agente, existe una opción mejor que ya está disponible: el plugin oficial `telegram@claude-plugins-official` de Claude Code.

- **Qué hace distinto**: en vez de un proceso separado con sus propios manejadores de comando (`/comprar`, `/vender`, etc.) que luego tendría que "hablarle" a Claude Code de alguna forma, este plugin corre un servidor MCP (con `bun`, no Node) que conecta los mensajes de Telegram **directamente al contexto de la sesión de Claude Code**. El asistente ve el mensaje entrante como parte de la conversación y responde con las herramientas que el plugin expone: `reply`, `react`, `edit_message`.
- **Botones — agregados a mano, no venían de fábrica**: el plugin de fábrica no expone una herramienta de teclado inline para uso general — solo tenía `InlineKeyboard` cableado internamente a su propio flujo de aprobación de permisos de Claude Code (`perm:allow:<id>`/`perm:deny:<id>`). Se extendió `server.ts` (fuera de este repo, ver advertencia abajo) para agregar un tool genérico `send_buttons({chat_id, text, buttons, reply_to?})`: manda hasta 8 botones, y un tap llega de vuelta como un mensaje de canal normal (`"Pressed button: <value>"`), con el mismo nivel de seguridad que los botones de permiso (solo remitentes en la lista blanca de DM). Este es el mecanismo recomendado para el patrón Aprobar/Rechazar de una orden real — ver CLAUDE.md para el detalle exacto del diff y cómo reaplicarlo si una actualización del plugin lo sobreescribe.
- **Control de acceso ya construido y más robusto que lo descrito en 4.3**: políticas `pairing`/`allowlist`/`disabled`, IDs numéricos, soporte de grupos con detección de menciones, todo en `~/.claude/channels/telegram/access.json`, gestionado con slash commands (`/telegram:access ...`) — cubre lo mismo que la sección 4.3 recomendaba construir a mano (lista blanca de `chat_id`), pero ya implementado y con más casos cubiertos (grupos, mención requerida, pareo).
- **Sigue siendo long polling** (vía grammY internamente, el mismo paquete recomendado en 4.2) — el análisis de la sección 4.1 sobre por qué polling y no webhook sigue aplicando, solo que ya no hay que escribirlo.
- **Requisito nuevo**: `bun` como runtime del servidor MCP del plugin (no lo necesita el resto del proyecto, que sigue en Node.js) — lo instala `scripts/wsl-setup.sh`.
- **Configuración**: son slash commands dentro de una sesión interactiva de `claude` (`/plugin install`, `/telegram:configure`, `/telegram:access ...`), no algo que un script pueda automatizar — ver CLAUDE.md para los pasos exactos.
- **⚠️ Riesgo de mantenimiento del fork de `send_buttons`**: vive en `~/.claude/plugins/marketplaces/claude-plugins-official/external_plugins/telegram/server.ts`, un archivo global gestionado por el marketplace de plugins, fuera del control de versiones de este repo. Una futura actualización del plugin puede sobreescribirlo sin avisar — si `send_buttons` deja de estar disponible, hay que revisar CLAUDE.md y volver a aplicar el cambio.
...
|---|---|---|
| Despliegue | Proceso Node.js/TypeScript dentro de WSL2 (Ubuntu), aislado de la instalación real de Windows (ver sección 6.6) | Sin costo de nube — ya era gratis. |
| Agente / LLM | **Claude Code directamente — sin enrutador entre APIs ni modelo local.** Decisión final (ver sección 3.6): al ser Claude Code el agente, no aplica la rotación Groq/Gemini/OpenRouter ni Ollama investigadas más abajo para el escenario de un bot autónomo separado. | El costo es el de tu uso normal de Claude Code, no un cargo nuevo por token específico de este proyecto. |
| Análisis técnico | `trading-signals` v8.2.0 (TS) | Ya era gratis — software libre, sin cambios. |
| Embeddings | **`Xenova/bge-m3`** (ONNX vía `@huggingface/transformers`, Node.js, CPU) — 1024 dim. **Construido y probado** en `scripts/rag/embeddings.ts`. | Gratis e ilimitado, sin llamadas a API, sin Ollama. Sin equivalente gratuito afinado a finanzas todavía — se pierde la ventaja de voyage-finance-2. |
| Reranking | **`Xenova/bge-reranker-base`** (ONNX, CPU) — **no** `bge-reranker-v2-m3`: confirmado en vivo el 5-ago-2026 que ese modelo no tiene conversión ONNX publicada. Construido y probado en `scripts/rag/reranker.ts`. | Gratis. Saltarse el reranking del todo pierde ~17% de recall relativo en documentos financieros (0.816 con reranker vs 0.695 sin él) — vale la pena correrlo localmente, el costo de cómputo es trivial a esta escala. |
| Vector DB | **Qdrant autohospedado dentro de WSL2 vía Docker Engine nativo** (no Docker Desktop, ver 6.6) | Gratis, sin límites de uso, sin riesgo de que un proveedor borre el cluster por inactividad (ver 8.1 sobre por qué evitar el free tier de Qdrant Cloud). |
...
| Bot de mensajería | Plugin oficial `telegram@claude-plugins-official` de Claude Code (usa grammY internamente, modo long polling) — ver sección 4.5 | Ya era gratis; además ahorra construir y mantener un bot propio. |
...
> **Nota (decisión posterior, ver 3.6)**: el punto sobre rotación Groq/Gemini/OpenRouter + Ollama de abajo describe el escenario de un bot autónomo separado — ya no aplica, porque Claude Code es el agente directamente. Se deja como referencia por si el diseño cambia en el futuro.

- ~~**El LLM es el punto más frágil.**~~ Estimación de volumen para ese escenario descartado (15 tickers × ~6 llamadas por ticker entre analistas+debate+riesgo × 3 revisiones/día) ≈ **270 llamadas/día**, o de forma más conservadora 150-500/día. Groq + Gemini + OpenRouter combinados **cubrían ese volumen en el total diario**, pero: (a) los picos al abrir el mercado (evaluar muchos tickers en pocos minutos) sí pueden golpear los límites por minuto de cada proveedor individual; (b) el listado de modelos gratuitos de OpenRouter rota mensualmente; (c) ninguno de estos proveedores publica una garantía de disponibilidad para su nivel gratuito — Gemini ya recortó sus cuotas gratuitas 50-80% en diciembre de 2025 y quitó Pro del nivel gratuito en abril de 2026, sin aviso previo.
- **Advertencia de términos de servicio que ya no aplica pero queda documentada**: la política de uso prohibido de Gemini restringe explícitamente pipelines automatizados **sin supervisión humana en dominios de alto riesgo, incluyendo finanzas** — relevante solo si algún día se vuelve a depender de Gemini para el razonamiento. Cohere queda descartado por completo en cualquier escenario: su clave de prueba está explícitamente prohibida para uso de producción.
...
- **Autohospedar Qdrant dentro de WSL2** vía Docker Engine nativo (no Docker Desktop, ver 6.6) — su binario nativo de Windows no tiene soporte oficial, usar su imagen Docker es el camino recomendado en cualquier caso.
...
- **`send_buttons` es un fork local del plugin de Telegram, no algo de fábrica** — vive en un archivo global fuera de este repo (`~/.claude/plugins/.../telegram/server.ts`) y una actualización del plugin puede sobreescribirlo sin avisar. Revisar que siga apareciendo en la lista de herramientas después de cualquier `/plugin update`; si no, reaplicar el cambio (diff descrito en sección 4.5 y en CLAUDE.md).
...
1. Conseguir el consentimiento por escrito de tu familiar/amigo (aunque sea un mensaje/correo simple) antes de tocar su cuenta — ver plantilla de puntos en la sección 6.4.
2. Confirmar con él si su PC va a quedarse encendida de forma continua (para uptime alto) o si el sistema es más bien algo que se revisa cuando la prende — esto decide si vale la pena configurar arranque automático de la distro WSL2 desde el día uno.
3. Coordinar el momento de la autorización OAuth inicial con Robinhood dentro de WSL2 (necesita su teléfono con la app de Robinhood y su login — sección 6.2/6.6).
4. Configurar de verdad el plugin de Telegram: token real vía @BotFather, `/plugin install`, `/telegram:configure`, pareo, y `/telegram:access policy allowlist` (pasos en la sección 4.5 y en CLAUDE.md).
...
7. ~~Construir el pipeline RAG~~ — **hecho** (`scripts/rag/`, probado con datos reales el 5-ago-2026, ver CLAUDE.md). Falta conectarlo a fuentes de datos reales (Finnhub, GDELT, Agent Reach, SearXNG) — hoy se alimenta a mano.
8. Definir el mecanismo de monitoreo remoto y actualización de código antes de dejarlo operando con dinero real sin que estés presente.
9. Prototipar el flujo Telegram (plugin oficial) → confirmación conversacional → Robinhood MCP de punta a punta con una sola señal simulada, antes de construir el resto del pipeline.
10. Si esto llegara a crecer más allá de esta única persona de confianza sin cobro (más personas, o cualquier forma de compensación), volver a evaluar el ángulo regulatorio — idealmente con una consulta legal puntual — antes de escalarlo.
